...
export class AuthService {
  ...
  register(dto: Partial<UserRegisterDTO>): Observable<UserResDTO> {
    return this.http.post<any>(
      'http://localhost:8080/e_banking_2_war_exploded/api/auth/register',
      dto,
  ...
  login(dto: Partial<UserLoginDTO>): Observable<UserResDTO> {
    return this.http.post<any>(
      'http://localhost:8080/e_banking_2_war_exploded/api/auth/authenticate',
      dto,
      { withCredentials: true }
    );
  }
  logout(): void {
    ...
    this.http.post(
      'http://localhost:8080/e_banking_2_war_exploded/api/auth/logout',
      {},
      { withCredentials: true }
    ).subscribe({
      next: () => {
